...
- [ ] Phase 1：合并准备
  ...
  - [ ] 运行 `git fetch upstream --prune` 并记录目标 commit（817b854）
  - [ ] `pnpm i && pnpm build`，确保当前基线可构建后再进行合并
- [ ] Phase 2：上游合并与冲突解决
  - [ ] 执行 `git merge upstream/main`，按 merge.md 决策矩阵处理（API/业务 ours，组件/样式 theirs）
  - [ ] 删除 i18n、快速入口、通知/聊天、示例/演示页面等禁止回归内容
  - [ ] 恢复多租户登录、平台范围、权限树等二开特性
- [ ] Phase 3：验证与文档
  - [ ] 更新 README/.env/src/mock/upgrade/changeLog 版本信息
  - [ ] `pnpm build` + lint 系列（lint、lint:prettier、lint:stylelint）
  - [ ] 更新 `.agentdocs/index.md` 当前任务状态，完成后移动任务文档到 done 目录
