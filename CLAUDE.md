...
改动时随时注意查看一下源项目的实现方式, 避免你本地改动不一致导致下次同步源项目时更加的困难, 比如说源项目是几个参数, 你这里也应该是几个参数, 而不是你这里改成了一个对象, 这样就会导致下次同步源项目时, 你这里的改动就会变得更加复杂, 所以说你在改动时, 一定要注意查看一下源项目的实现方式, 避免你本地改动不一致导致下次同步源项目时更加的困难.
